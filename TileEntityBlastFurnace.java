...
import com.doggie.RhoduimMod.blocks.machines.blast.BlastFurnace;
...
public class TileEntityBlastFurnace extends TileEntity implements ITickable
{
	...
	{	
		...
		else
		{
			...
			{
				ItemStack output = BlastFurnaceRecipes.getInstance().getBlastResult(inputs[0], inputs[1]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = BlastFurnaceRecipes.getInstance().getBlastResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));	
